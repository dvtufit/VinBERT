...
class BERTVin(nn.Module):
    ...
    def __init__(self, bert_model=None):
        ...
        self.mlp = nn.Sequential(
            ...
            nn.Linear(self.qwen_vocab_size, self.bert_size), 
            nn.GELU(),
            nn.Linear(self.bert_size, self.bert_size),
            nn.LayerNorm(self.bert_size, eps=1e-5, elementwise_affine=True),
            nn.Dropout(p=0.1)
        )
        ...
        self.classifier = nn.Sequential(
            nn.Linear(self.bert_size, self.num_classes),
        )
...
class BERTInternVLChatModel(PreTrainedModel):
